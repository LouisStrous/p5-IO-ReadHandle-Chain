...
like($@, qr/^Can't locate object method "PRINT" via package "IO::ReadHandle::Chain"/, 'print fails');

$@ = '';
eval { printf $cfh '%s', 'foo'; };
like($@, qr/^Can't locate object method "PRINTF" via package "IO::ReadHandle::Chain"/, 'printf fails');

$@ = '';
eval { $cfh->syswrite($buffer, 5) };
like($@, qr/^Can't locate object method "syswrite" via package "IO::ReadHandle::Chain"/, 'printf fails');

# seeking fails

$@ = '';
eval { $cfh->seek(0, 0) };
like($@, qr/^Can't locate object method "seek" via package "IO::ReadHandle::Chain"/, 'printf fails');
