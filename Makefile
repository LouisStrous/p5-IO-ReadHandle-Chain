# This Makefile is for the IO::ReadHandle::Chain extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/IO/ReadHandle/Chain.pm]
...
#     NAME => q[IO::ReadHandle::Chain]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/IO/ReadHandle/Chain.pm]
#     clean => { FILES=>q[IO-ReadHandle-Chain-*] }
...
MAN3EXT = 3pm
INSTALLDIRS = site
INSTALL_BASE = /home/strous/perl5
DESTDIR = 
PREFIX = $(INSTALL_BASE)
...
MAKE = make
FULLEXT = IO/ReadHandle/Chain
BASEEXT = Chain
PARENT_NAME = IO::ReadHandle
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/IO/ReadHandle/Chain.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/IO/ReadHandle
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/IO/ReadHandle
...
TO_INST_PM = lib/IO/ReadHandle/Chain.pm
...
DISTNAME = IO-ReadHandle-Chain
DISTVNAME = IO-ReadHandle-Chain-0.01
...
manifypods : pure_all config  \
	lib/IO/ReadHandle/Chain.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) -u \
	  lib/IO/ReadHandle/Chain.pm $(INST_MAN3DIR)/IO::ReadHandle::Chain.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Chain several sources through a single file read handle'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Louis Strous <lstrous@cpan.org>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: IO-ReadHandle-Chain' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Chain several sources through a single file read handle",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Louis Strous <lstrous@cpan.org>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "IO-ReadHandle-Chain",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="IO-ReadHandle-Chain" VERSION="0.01">' > IO-ReadHandle-Chain.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Chain several sources through a single file read handle</ABSTRACT>' >> IO-ReadHandle-Chain.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Louis Strous &lt;lstrous@cpan.org&gt;</AUTHOR>' >> IO-ReadHandle-Chain.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> IO-ReadHandle-Chain.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> IO-ReadHandle-Chain.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-thread-multi-5.24" />' >> IO-ReadHandle-Chain.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> IO-ReadHandle-Chain.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> IO-ReadHandle-Chain.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> IO-ReadHandle-Chain.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/IO/ReadHandle/Chain.pm' 'blib/lib/IO/ReadHandle/Chain.pm' 
